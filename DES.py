...
def generate_subkeys(key):
    key_bin = to_bin(struct.unpack('>Q', key)[0])  
...
def feistel(R, subkey):
    ...
    R_xor = bin(to_int(R_expanded) ^ to_int(subkey))[2:].zfill(48)  
    output = ''
...
def des_encrypt_block(block, subkeys):
    block = to_bin(struct.unpack('>Q', block)[0])  
    block = permute(block, IP)  
    L, R = block[:32], block[32:]
    
    for i in range(16):
        temp = R
        R = bin(to_int(L) ^ to_int(feistel(R, subkeys[i])))[2:].zfill(32)  
        L = temp
    
    cipher_block = permute(R + L, FP)  
    return struct.pack('>Q', to_int(cipher_block))
...
def des_decrypt_block(block, subkeys):
    ...
    return struct.pack('>Q', to_int(plain_block))
...
def encrypt_file(input_file, output_file, key):
    ...
    with open(input_file, 'rb') as f_in, open(output_file, 'wb') as f_out:
        ...
        while True:
            ...
            f_out.write(des_encrypt_block(block, subkeys))
...
def decrypt_file(input_file, output_file, key):
    ...
    with open(input_file, 'rb') as f_in, open(output_file, 'wb') as f_out:
        ...
        while True:
            ...
            if len(block) == 0:
                break
            f_out.write(des_decrypt_block(block, subkeys))
